...
// EventsData Component
const EventsData = ({ event, onClose }) => {
  const { watyLearningData } = FetchWatyLearningHomepageData();
  const eventsFormData = watyLearningData?.form;

  const [loading, setLoading] = useState(false); // State to track loading

  // Shared function to handle form submission
  const handleEnrollClick = async (eventSubmit) => {
    ...
    const senderEmail = eventsFormData?.contactEmail;
    const emailContent = eventsFormData?.emailContent;
    const emailContentForUs = eventsFormData?.emailContentForUs;
      ...
      >
        <div className="max-sm:mt-0 flex flex-col items-center gap-4">
          ...
          <input
            ...
            name="name"
            required
            placeholder={eventsFormData?.nameTitle || "Name"}
          ...
          <input
            ...
            name="email"
            required
            placeholder={eventsFormData?.emailTitle || "Email"}
...
<input
            ...
            name="phone"
            required
            placeholder={eventsFormData?.phoneTitle || "Phone Number"}
            className="w-[350px] font-secondary text-sm max-sm:text-[12px] bg-white/5 py-2 pl-2 rounded-md mb-5 border border-white/5 max-sm:mx-auto max-sm:w-[250px]"
            style={{
